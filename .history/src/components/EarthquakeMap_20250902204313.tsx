...
export default function EarthquakeMap() {
  ...
  if (status === "loading") return <div className="text-center py-8">Loading map...</div>;
  if (!data) return <div className="text-center py-8">No data available.</div>;

  const center: LatLngExpression = [20, 0]; // Explicitly typed as LatLngExpression
  ...
  return (
    ...
    >
      ...
      {data.features.map((feature: any) => (
        <CircleMarker
          key={feature.id}
          center={[feature.geometry.coordinates[1], feature.geometry.coordinates[0]] as LatLngExpression}
        ...
        >
          <Popup>
            <div>
              ...
              <div>
                <span className="font-semibold">Magnitude:</span> {feature.properties.mag}
              </div>
              <div>
                <span className="font-semibold">Location:</span> {feature.properties.place}
              </div>
              <div>
